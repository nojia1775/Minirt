...
void	test_rotation_matrix(void)
{
	...
	if ((int)(tup2->coor[0] + 0.5) != 0.0 || (int)(tup2->coor[1] + 0.5) != 0.0 || (int)(tup2->coor[2] + 0.5) != 1.0)
		...
		printf("[KO] rotation  test 2, x : %f, y : %f, z : %f\n", tup3->coor[0], tup3->coor[1], tup3->coor[2]);
...
void	test_normal_transformation(void)
{
	...
	// new_center = multiplying_matrix_tuple(&transform, sphere.xyz);
	// sphere.xyz = *new_center;
	// point = create_tuple2(0.0, 1.70711, -0.70711, 1);
	// vector = normal_vector_sphere(sphere, point);

	sphere.xyz = create_tuple2(0.0, 0.0, 0.0, 1);
	transform2 = scaling(1, 0.5, 1);
	transform3 = rotation_z(PI/5);
	transform4 = multiplying_4X4_matrix(&transform2, &transform3);
	display_canva(&transform4);
	new_center = multiplying_matrix_tuple(&transform4, sphere.xyz);
	sphere.xyz = *new_center;
	point2 = create_tuple2(0.0, sqrt(2) / 2, -sqrt(2) / 2, 1);
	vector2 = normal_vector_sphere(sphere, transform4, point2);
	printf("x : %f, y : %f, z : %f\n", vector2.coor[0], vector2.coor[1], vector2.coor[2]);
}
